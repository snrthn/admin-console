...
import { Table as MyTable } from 'antd';
...
const Table = React.forwardRef<HTMLTableElement, TableProps>(
  (
    ...
    const coluList1 = (children[0]?.props?.children[0]?.props?.children || []).map(item => {
      return {
    ...
    const coluList2 = (children?.props?.children[0]?.props?.children[0]?.props?.children || []).map(item => {
      return {
        title: item.props.children,
        dataIndex: item.key,
        key: item.key,
    ...
    const coluList = coluList2.length ? coluList2 : coluList1

    const dataList = (children?.props?.children[1]?.props?.children || []).map(item => {
      let retObj = {};

      item.props.row.allCells.forEach(cell => {
        console.log(cell)
        retObj['header_' + cell.column.id] = ((typeof cell.value === 'string' || !cell.value) ? cell.value : cell.value[0]?.title) || '-'
      })
    ...
    return (
      <div className={`flex flex-col ${containerClassName}`}>
        ...
        <div className="relative">
          ...
          {/* <MyTable dataSource={ dataList } columns={ coluList } /> */}
