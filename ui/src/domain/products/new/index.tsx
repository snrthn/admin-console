...
import MteForm, {MteFormType} from "../../../components/forms/product/mte-form";
...
const NewProduct = ({ onClose }: Props) => {
  ...
  const onSubmit = (publish = true) =>
    handleSubmit(async (data) => {
      ...
      if (data.mte?.detail_id.length) {
        payload['detail_id'] = data.mte.detail_id
      }
        ...
        <FocusModal.Main className="no-scrollbar flex w-full justify-center py-16">
          <div className="small:w-4/5 medium:w-7/12 large:w-6/12 max-w-[700px]">
            <Accordion defaultValue={["general"]} type="multiple">
              ...
              <Accordion.Item title={t("图文详情", "图文详情")} value="mte">
                <MteForm form={nestedForm(form, "mte")} />
              </Accordion.Item>
            </Accordion>
          </div>
        </FocusModal.Main>
