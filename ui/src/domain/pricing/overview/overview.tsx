...
const PriceListOverview = () => {
  ...
  const { price_lists, count, isLoading, isError } = useAdminPriceLists(
    {
      limit: PAGE_SIZE,
      offset: (tableParams.pagination.current - 1) * PAGE_SIZE,
      ...
      expand: "customer_groups,customer_tags",
    },
    {
  ...
  React.useEffect(() => {
    ...
    _params.pagination.total = count;
    setTableParams(_params);
  }, [count]);
  ...
  const columns_: TableProps<any>['columns'] = [
    {
        title: 'Name',
        dataIndex: 'name',
        key: 'name',
        render: (text: string,record) => <a onClick={() => {
          navigate(`/a/pricing/${record.id}`)
        }}>{text}</a>,
    },
    {
        title: 'Description',
    ...
    },
    {
      ...
      dataIndex: 'status',
      key: 'status',
      render: (_, record) => { // Update the type of the _row variable
        let _row = {...record, original: record}
        return <PriceListTableRowStatus record={_row} />;
      },
    },
    {
      title: 'Product Type',
      ...
      key: 'product_type',
      render: (text) => text=="product" ? "产品" : "产品套餐",
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => { // Update the type of the _row variable
        let _row = {...record, original: record}
        return <PriceListTableRowActions row={_row} />;
      },
    },
  ];

  type PriceListTableRowStatusProps = {
    record:PriceList
  }
  const PriceListTableRowStatus = ({ record }:PriceListTableRowStatusProps) => {
    ...
    const isDraft = record.status === PriceListStatus.DRAFT
...
const columns = [
  ...
  columnHelper.accessor("customer_tags", {
