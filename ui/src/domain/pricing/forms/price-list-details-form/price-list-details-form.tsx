...
import {useCustomerTagGroup} from "../../../../hooks/customer/use-admin-customer-tags";
import {Radio, Tabs, Flex, RadioChangeEvent} from "antd"
...
const PriceListCustomerGroups = ({
  ...
  const [tagOrGroup, setTagOrGroup] = React.useState<string>("Group")
  ...
  const { customer_groups, count, isLoading, isError } = useCustomerTagGroup(
    {
      limit: PAGE_SIZE,
      offset,
      expand: "customers",
  ...
  React.useEffect(() => {
    ...
    register(path("customer_tags.ids"))
  }, [register, path])
  ...
  React.useEffect(() => {
    if (tagOrGroup === "Group") {
      ...
      setValue(path("customer_tags.ids"), Object.keys(rowSelection), {
        shouldDirty: true,
        shouldTouch: true,
      })
    }
        ...
        <Collapsible.Content>
          <div className="pt-6">
            <Container className="overflow-hidden p-0 ">
              ...
              >
                <Radio.Group defaultValue="Group" size="small" onChange={onRadioChange}>
                  <Radio.Button value="Group">{ t("details-groups", "Group")}</Radio.Button>
                  <Radio.Button value="tag">{ t("details-tags", "Tag")}</Radio.Button>
                </Radio.Group>
                <Heading>
                  {tagOrGroup === "Group" ? t("details-groups", "Group") : t("details-tags", "Tag")}
                </Heading>
                <div
