import {useAdminCustomerDetails} from "../../../hooks/customer/use-admin-customers"
...
const CustomerDetail = () => {
  ...
  const { customer, isLoading, error } = useAdminCustomerDetails(id!)
        ...
        <Section>
          ...
          <div className="mt-6 flex space-x-6 divide-x">
            ...
            <div className="flex flex-col pl-6">
              ...
              <div className="max-w-[200px] truncate">
                {customer.tags?.map((item: {value: string}) => item.value.trim()).join(",") || "N/A"}
              </div>
            </div>
            <div className="flex flex-col pl-6">
              <div className="inter-smaller-regular text-grey-50 mb-1">
                {t("details-group", "Groups")}
              </div>
              <div className="max-w-[200px] truncate">
                {customer.groups?.map((item: {name: string}) => item.name.trim()).join(",") || "N/A"}
              </div>
            </div>
          </div>
        </Section>
